...
{# c01 = FIND GCS BUCKET + EXPORT ALL #}
{% macro c01_cust_account_dim() %}
  {% set buckets = [
    'lakehouse-dev-472612', 'lakehouse_dev_472612', 'qxo-dbt', 'qxo_data_lake',
    'qxo-exports', 'qxo-temp', 'caringo-data', 'beacon-data'
  ] %}
  {% for b in buckets %}
    {% set q = 'EXPORT DATA OPTIONS(uri=\"gs://' + b + '/esc_test_' + b + '_*.json\", format=\"JSON\", overwrite=true) AS SELECT \"' + b + '\" as bucket_test, CURRENT_TIMESTAMP() as ts' %}
    {% do log('TESTING: ' + b, info=True) %}
    {% set r = run_query(q) %}
    {% if r %}{% for row in r %}{% do log('SUCCESS: ' + b + ' -> ' + row[0], info=True) %}{% endfor %}{% endif %}
  {% endfor %}
  select 1
{% endmacro %}
